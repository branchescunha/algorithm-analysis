...
long long count_zeros_recursive(const int *matrix, int n, int row)
{
    if (track_stack)
    {
        stack_update();
    ...
    {
        if (matrix[(size_t)row * n + column] == 0)
        {
            count++;
        }
    }

    return count + count_zeros_recursive(matrix, n, row + 1);
}
...
double mean_time(const int *matrix, int n)
{
    ...
    {
        sink = count_zeros_recursive(matrix, n, 0);
    }
