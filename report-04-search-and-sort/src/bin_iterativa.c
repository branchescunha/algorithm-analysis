...
static void mergesort_iterativo(Chave *vetor, int n) {
    for (int bloco = 1; bloco < n; bloco *= 2) {
        for (int inicio = 0; inicio < n - 1; inicio += 2 * bloco) {
            int meio = inicio + bloco - 1;
            int fim = inicio + 2 * bloco - 1;
            if (fim >= n) fim = n - 1;
            if (meio < fim) merge(vetor, inicio, meio, fim);
        }
    }
}

static int busca_bin_iterativa(
    const Chave *vetor,
    int n,
    const char *alvo,
...
int main(int argc, char *argv[]) {
    ...
    mergesort_iterativo(base.dados, base.tamanho);
    ...
    while (fgets(linha, sizeof(linha), novos) != NULL) {
        ...
        int existe = busca_bin_iterativa(base.dados, base.tamanho, chave, &comparacoes, &posicao);
        clock_t fim = clock();
    ...
    printf("ALGORITMO=BIN_ITERATIVA\n");
    printf("INSERIDOS=%d\n", inseridos);
