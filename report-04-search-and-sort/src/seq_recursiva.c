...
static int busca_seq_recursiva(
    const Chave *vetor,
    int n,
    int indice,
    const char *alvo,
    ...
    (*comparacoes)++;
    if (strcmp(vetor[indice].valor, alvo) == 0) return 1;
    return busca_seq_recursiva(vetor, n, indice + 1, alvo, comparacoes);
}
...
int main(int argc, char *argv[]) {
    ...
    while (fgets(linha, sizeof(linha), novos) != NULL) {
        ...
        int existe = busca_seq_recursiva(base.dados, base.tamanho, 0, chave, &comparacoes);
        clock_t fim = clock();
    ...
    printf("ALGORITMO=SEQ_RECURSIVA\n");
    printf("INSERIDOS=%d\n", inseridos);
